...
## Access
  ...
  Scenario Outline: Logged-in, only the request author or an admin can access the request archive form.
...
## Form visibility.
  Scenario Outline: When reaching the request listing, only the request author or an admin should see the request archive form
    Given I am logged in as user "<user>"
    When I am on "<url>"
...
## Form submits.
  ...
  Scenario: When archiving someone else requests, a email should be sent to the request's author and I should be redirect to the listing of requests.
    ...
    Then A mail as been sent to "manager+lausanne@antistatique.net" with subject "qs.mail.request.archived.subject Resoli"
    Then the url should match "/sharing/1/requests"
    And I should see "qs_sharing.collection.request.archive.success" in the ".alert" element
    Then I should see 2 "#requests-accordion .card-list-item" elements
