Feature: Event Edit Form
  ...
  Scenario: When reaching the Edit form of Event, the fields should be prefilled with entity values
    ...
    And the "#edit-date" field should be formatted "/[0-9]{2}.[0-9]{2}.[0-9]{4}/"
    And the "#edit-start-at" field should be formatted "/(([0-1][0-9])|([2][0-3])):([0-5][0-9])/"
    And the "#edit-end-at" field should be formatted "/(([0-1][0-9])|([2][0-3])):([0-5][0-9])/"
